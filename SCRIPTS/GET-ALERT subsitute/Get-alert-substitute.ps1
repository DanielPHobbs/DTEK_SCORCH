...
$Global:MyCredential = new-object -typename System.Management.Automation.PSCredential -argumentlist $username, $password		
$Global:MyCredential
...
$resultArray = Invoke-Command  -Session $Session -ScriptBlock {
...
Function Get-Scomalerts([int]$timeout=240)
{
...
##################     Main Calling section  ##################################
        ...
        AppendLog "Getting Alerts"
        Get-Scomalerts
		
        $AlertData = $FalertData
...
} #-Authentication CredSSP -Credential $MyCredential
